...
const { moderators, managers, creatorID} = require("../botconfig/main.json");
...
client.on("messageCreate", async (message) => {
  ...
  if (command.toggleOff) {
    ...
    if (!user.roles.cache.some((m) => moderators.includes(m.id))) {
      let modsOnly_embed = new MessageEmbed()
        .setTitle(
          `:x: | Only Server Moderators and Server Administrators Can Use That Command!`
        )
        .setColor("RED")
        .setFooter(`${clientname}`, `${clientavatar}`)
        .setTimestamp();
      return message.reply({ embeds: [modsOnly_embed] });
    }
  ...
  } else if (command.creatorOnly) {
    if (!message.author.id == "801384603704623115" ) {
      let creatorOnly_embed = new MessageEmbed()
        .setTitle(`:x: | Only Creators Can Use That Command!`)
        .setDescription(
          `My Creator: ${creatorID.map((v) => `<@${v}>`).join(",")}`
        )
        .setColor("RED")
        .setFooter(`${clientname}`, `${clientavatar}`)
        .setTimestamp();
      return message.reply({ embeds: [creatorOnly_embed] });
    }
