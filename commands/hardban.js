...
module.exports = {
  data: new SlashCommandBuilder()
    .setName("hardban")
    .setDescription("Hardban a user from Rosana.")
    .addUserOption((option) =>
      option.setName("user").setDescription("Who is the user you are banning?")
    )
  ...
  async execute(interaction) {
    ...
    const reason = interaction.options.getString("reason");
      ...
      const auditChannel = interaction.guild.channels.cache.get(auditLogs);
      const shameChannel = interaction.guild.channels.cache.get(shameLogs);
    ...
    if (user && reason) {
      (await user.send({ embeds: [banned_Embed] }))
        ? console.log("I messaged the user.")
        : console.log("I couldn't message the user.");
      await user
        .ban({
          reason: reason,
        })
        .then(() => {
          ...
          await(shameLogs.send({ embeds: [publicAudit_Embed] }));
